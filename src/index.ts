...
async function init(){
    try {
        ...
        server = app.listen(conf.port, () => console.log(`Listening on ${conf.port}`));
    } catch (err) {
        console.error(err);
    }
}
...
async function DBTest() {
    ...
    // PostgreSQL test
    await pgdb.query('SELECT * FROM cosmo.contacts WHERE name = $1', ['ljs'], (err, result) => {
        if (err) {
            process.exit(1);
        }
    ...
    try {
        ...
        const queryText = 'INSERT INTO cosmo.contacts(name,phone) VALUES($1, $2)';
